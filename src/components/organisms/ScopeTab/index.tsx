...
export const ScopeTab: React.FC = () => {
  const [dataSource, setDataSource] = useState<ScopeItem[]>(MOCK_SCOPE_DATA);
  ...
  const handleChange = (id: string, field: keyof ScopeItem, value: string) => {
    setDataSource((prev) =>
  ...
  const handleSave = () => {
    ...
    console.log('Scope data to save:', validData);
  ...
  const columns: ColumnsType<ScopeItem> = [
    {
      ...
      key: 'system',
      width: 200,
      render: (text: string, record: ScopeItem) => (
        <Select
    ...
    },
    {
      ...
      key: 'action',
      width: 80,
      render: (_: unknown, record: ScopeItem) => (
        <Button
          type="text"
          danger
          icon={<DeleteOutlined />}
  ...
  return (
    <div className="scope-tab">
      <div className="scope-content" ref={tableBodyRef}>
